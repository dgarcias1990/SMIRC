	...
	for instance in usuarios_nodos :
		...
		for ubicacion in ubicaciones:
			...
			for x in range(start,len(usuarios_nodos)+1):
				otrasLoc=list(localization.objects.raw('select id, usuario_id,latitud,longitud,altitud,charla,fechaHora from inicio_localization where usuario_id=%s and trunc(latitud,4)=%s and trunc(longitud,4)=%s and (fechaHora>=%s and fechaHora<=%s) LIMIT 1',[x,float("%.4f" % ubicacion.latitud),float("%.4f"%ubicacion.longitud),t1,t2]))
				if len(otrasLoc)>0:
				#print otrasLoc
					print len(otrasLoc)
					locate=otrasLoc[0]
